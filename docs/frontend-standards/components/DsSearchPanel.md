...
| Implementation | `DsSearchPanel` |
| 类型 | `business` |
| 状态 | `stable` |
| 当前路径 | `src/components/business/DsSearchPanel.vue` |

`DsSearchPanel` 用于后台列表页的标准筛选区，承载常用字段、查询、重置和更多筛选入口。它应保持紧凑、稳定、可扫描，避免每个页面重复实现搜索表单。

## 适用场景

- 后台列表页、台账页、报表页的顶部筛选区。
- 字段数量较多，需要常用字段外露、低频字段通过“更多字段”管理。
- 页面已经或准备使用 `DsDataTable`、`DsTableToolbar` 等列表页组件。
...
`DsSearchPanel` 内置字段必须使用 `DsCompactFieldControls`：

- `input` -> `DsCompactInput`
- `select` -> `DsCompactSelect`
- `date` -> `DsCompactDatePicker`
- `dateRange` -> `DsCompactRangePicker`
...
- `DsSearchPanel` 内部字段高度固定为 28px，不用于新增、编辑、审批等正式业务表单。
- 自定义字段应通过 `field-{key}` slot 实现，并保持 28px 高度和 12px 字号。
- 更多字段、保存筛选方案等弱操作放入 `actions` slot，不应改变查询和重置按钮位置。

## 与列表页组件的协作

- 放置在 `DsPageHeader` 下方、`DsTableToolbar` 上方。
- 筛选结果通过页面逻辑传给 `DsDataTable`。
- 表格列配置、密度、导出等能力不放在本组件内，应由 `DsTableToolbar` 和 `DsColumnConfigDrawer` 承担。

## 迁移建议

从历史 `AdvancedFilter` 或页面自写搜索表单迁移时：

1. 梳理字段为 `DsSearchField[]`。
...
AI 新增或迁移列表页搜索区时，必须先检查 `DsSearchPanel` 是否满足需求。除非存在明确业务差异，否则不得在页面内重新实现搜索表单、查询按钮、重置按钮和字段控件高度样式。
